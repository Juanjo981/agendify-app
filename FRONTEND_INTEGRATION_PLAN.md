...
**Objetivo:** Integrar la página de Actividad, completar la integración funcional de notificaciones y reutilizar la bitácora real donde ya exista surface activa.

**Alcance:**
- Página de actividad con feed real filtrable
- Notificaciones: listado real y marcado individual como leída desde la campana
- Reutilización de `historial-eventos` como fuente global sin tocar la UI visual existente

**Pantallas afectadas:**
- `actividad.page.integrated.ts` — feed de actividad
...
- Nuevo: `services/actividad-api.service.ts`

**Endpoints involucrados:**
| Método | Ruta | Propósito |
|--------|------|-----------|
| `GET` | `/api/historial-eventos` | Feed global de actividad (agrupado y filtrado en frontend) |
| `GET` | `/api/notificaciones` | Listado completo |
| `PATCH` | `/api/notificaciones/{id}/leida` | Marcar como leída (contrato operativo pendiente de canonización documental) |
| `POST` | `/api/notificaciones/marcar-todas-leidas` | Marcar todas (pendiente de endpoint/documentación + affordance visual) |

**Modelos involucrados:**
- `NotificacionDto` — ya definido en Fase 7
- Nuevo: `HistorialEventoDto` / `ActividadFeedItem` — mapper desde `/api/historial-eventos`

**Dependencias previas:** Fase 7 (Dashboard con notificaciones parciales)

**Cambios técnicos:**
1. Crear `ActividadApiService` sobre `/api/historial-eventos`
2. Conectar filtros de tipo de actividad (agenda, equipo, sistema, reprogramar)
3. Agrupar por fecha en frontend sin cambiar la UI existente
4. Marcar notificaciones como leídas al hacer click en el dropdown
5. Dejar documentada la acción masiva como pendiente si no existe endpoint/affordance canónico
...
**Criterios de terminado:**
- [x] Página de actividad muestra feed real
- [x] Filtros de tipo funcionan
- [x] Notificaciones se pueden marcar individualmente como leídas
- [ ] "Marcar todas" funciona
- [x] Feed agrupado por fecha visualmente correcto
...
3. Notificaciones integradas con lectura individual y documentación explícita de brechas restantes
...
- [x] Empty states funcionales mediante datasets vacíos / placeholders no disruptivos
...
| 10 — Actividad/Notif. | Baja | Media | Fase 7 | Medio | Feed, Notificaciones | Parcial | Sí | ✅ Completada (acción masiva pendiente documentada) |
...
## Actualizaci�n Fase 11 (2026-04-07)
- EstadisticasApiService creado y conectado a dashboard, citas, ingresos, pacientes, insights, caja diaria y reportes.
- EstadisticasMockService ya no se inyecta en ninguna vista activa del m�dulo.
- Los filtros de rango/fecha se traducen a query params reales y las sub-vistas escuchan el estado global de filtros.
- La exportaci�n qued� conectada con soporte tolerante para blob directo o JSON con url/mensaje.

import { IngresoPorPeriodo, PeriodoCitas } from '../../models/estadisticas.model';
import { MESES_ABREV_ES, resolveAnioVistaAnual } from '../chart-citas-por-periodo/citas-periodo-buckets.util';

const emptyIngreso = (fecha: string, label: string): IngresoPorPeriodo => ({
  fecha,
  label,
  total: 0,
  efectivo: 0,
...
function mergeIngresos(rows: IngresoPorPeriodo[], periodo: PeriodoCitas): Map<string, IngresoPorPeriodo> {
  const m = new Map<string, IngresoPorPeriodo>();
  for (const b of rows) {
    ...
    if (!prev) {
      ...
      m.set(k, {
        ...
        efectivo: prev.efectivo + b.efectivo,
        transferencia: prev.transferencia + b.transferencia,
        tarjeta: prev.tarjeta + b.tarjeta,
        pendiente: prev.pendiente + b.pendiente,
      });
    }
  }
  return m;
...
export function normalize7DaysIngresos(barrasApi: IngresoPorPeriodo[], hoy = new Date()): IngresoPorPeriodo[] {
...
export function normalizeWeeksIngresos(barrasApi: IngresoPorPeriodo[], fechaRefYmd: string): IngresoPorPeriodo[] {
  const ref = parseYmdLocal(fechaRefYmd.slice(0, 10)) ?? startOfTodayLocal();
  const y = ref.getFullYear();
  const mo = ref.getMonth();
  const first = new Date(y, mo, 1, 12, 0, 0, 0);
  const last = new Date(y, mo + 1, 0, 12, 0, 0, 0);
  ...
  const byKey = mergeIngresos(barrasApi, 'semana');
...
export function normalizeMonthsIngresos(
  barrasApi: IngresoPorPeriodo[],
  fechaDesde: string,
  fechaHasta: string,
): IngresoPorPeriodo[] {
  const anio = resolveAnioVistaAnual(fechaDesde, fechaHasta);
  const byKey = mergeIngresos(barrasApi, 'mes');
  const out: IngresoPorPeriodo[] = [];
  for (let mo = 1; mo <= 12; mo++) {
    const key = `${anio}-${String(mo).padStart(2, '0')}`;
    const hit = byKey.get(key);
    const label = MESES_ABREV_ES[mo - 1];
    if (!hit) {
      out.push(emptyIngreso(key, label));
    } else {
...
export function rellenarSerieIngresosPorPeriodo(
  barrasApi: IngresoPorPeriodo[],
  fechaDesde: string,
  fechaHasta: string,
  ...
  switch (periodo) {
    case 'dia':
      return normalize7DaysIngresos(barrasApi, new Date());
    case 'semana':
      return normalizeWeeksIngresos(barrasApi, fechaHasta || formatYmd(startOfTodayLocal()));
    case 'mes':
      return normalizeMonthsIngresos(barrasApi, fechaDesde, fechaHasta);
    default:
      return barrasApi;
  }
}

...
> Última actualización: Fase 11 — Estadísticas y Reportes (7 de abril de 2026)
...
| F14 | Fase 7/10 / Dashboard-Notificaciones | El frontend ya intenta persistir lectura individual con `PATCH/POST /api/notificaciones/{id}/leida`, pero la API Reference vigente no documenta un endpoint canónico ni un campo estable de lectura (`leida`, `fecha_leida`, etc.) | La apertura desde la campana puede quedar dependiente de un contrato implícito o desactualizado | Publicar el endpoint oficial de lectura y el DTO fuente de verdad para estado `pendiente/leida` | ?? Media | ? Pendiente |
...
| F18 | Fase 9 / Equipo | La documentación de Fase 9 sigue mencionando `/api/equipo`, pero la API real documentada expone `/api/recepcionistas` y subrutas `/permisos` / `/activo` | El frontend tuvo que implementar fallback de endpoints para soportar ambas referencias | Unificar plan/documentación hacia el contrato real de recepcionistas | ?? Media | ? Pendiente |
| F19 | Fase 9 / Equipo | El mapping exacto de permisos backend → frontend no está especificado de forma canónica; la API reference solo muestra ejemplos granulares como `puede_crear_citas` / `puede_editar_pacientes` | El frontend tuvo que usar un mapper heurístico para consolidar permisos visuales (`agenda`, `citas`, `pacientes`, `notasClinicas`, `configuracion`) | Documentar oficialmente la tabla de equivalencias o devolver un DTO ya agregado para la UI administrativa | ?? Media | ? Pendiente |
| F20 | Fase 10 / Actividad | El plan de integración seguía proponiendo `GET /api/actividad`, pero la referencia real publicada expone `GET /api/historial-eventos` como feed global reutilizable | El frontend tuvo que integrar la pantalla de actividad contra `historial-eventos` y mapearlo al view model visual actual | Unificar plan/API reference hacia una sola surface oficial para actividad global | ?? Media | ? Pendiente |
| F21 | Fase 10 / Notificaciones | La UI actual no tiene un control visible para “marcar todas como leídas” y la API Reference vigente tampoco documenta de forma canónica `POST /api/notificaciones/marcar-todas-leidas` | El flujo individual queda integrado desde la campana, pero la acción masiva no puede exponerse sin cambiar diseño ni asumir un endpoint no confirmado | Confirmar endpoint oficial y definir si se autoriza un affordance existente o una mínima adición visual para esa acción | ?? Media | ? Pendiente |
| F22 | Fase 11 / Estadísticas | No hay DTO canónico documentado para `GET /api/estadisticas/resumen`, `GET /api/estadisticas/citas`, `GET /api/estadisticas/ingresos`, `GET /api/estadisticas/pacientes` y `GET /api/estadisticas/reportes`; faltan nombres oficiales para series, rankings, resúmenes y previews (`conteo_por_estado`, `horas_mas_ocupadas`, `ranking_mas_citas`, `filas`, etc.) | El frontend tuvo que integrar con mappers tolerantes y no puede validar contrato exacto campo por campo solo con la documentación vigente | Publicar los DTOs fuente de verdad de estadísticas/reportes o ampliar `FRONTEND_API_REFERENCE.md` con ejemplos completos por endpoint | ?? Alta | ? Pendiente |
| F23 | Fase 11 / Estadísticas | No existe un endpoint documentado para poblar el filtro de profesionales del módulo de estadísticas | Para preservar diseño sin inventar opciones, el filtro solo puede operar de forma segura con “Todos los profesionales” hasta que exista contrato real | Exponer catálogo ligero de profesionales o documentar reutilización de un endpoint existente con sus campos mínimos (`id`, `nombre`) | ?? Media | ? Pendiente |
| F24 | Fase 11 / Reportes | `POST /api/estadisticas/reportes/exportar` no documenta forma de respuesta estable: blob directo vs JSON con `url`, `mensaje`, filename o `Content-Disposition` | La exportación se integró con manejo tolerante, pero la descarga final depende de un contrato backend ambiguo | Definir respuesta canónica de exportación y headers esperados para PDF/Excel | ?? Alta | ? Pendiente |
